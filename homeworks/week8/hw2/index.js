...
function getGames(callback) {
  fetch(`${apiUrl}/games/top?limit=${gamesLimit}`, requestOptions)
...
function getStreams(name, callback) {
  fetch(`${apiUrl}/streams?game=${encodeURIComponent(name)}&limit=${streamsLimit}&offset=${offset}`, requestOptions)
    .then(response => response.json())
    .then(data => callback(data))
    .catch(error => console.log('error', error));
}
...
getGames((games) => {
  ...
  getStreams(topGames[0], (data) => { renderStreams(data.streams); });
  mediaTitle.innerText = games.top[0].game.name;
});
...
document.querySelector('.nav__select').addEventListener('change', (e) => {
  ...
  getStreams(optionValue, (data) => { renderStreams(data.streams); });
});
...
document.querySelector('.media__button').addEventListener('click', () => {
  offset += 20;
  getStreams(mediaTitle.innerText, (data) => { renderStreams(data.streams); });
});
